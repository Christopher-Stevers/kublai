...
import { handleCatalogueReplicachePull } from "~/server/replicache/catalogue-sync";
...
export async function POST(request: Request) {
  ...
  try {
    const response = await handleCatalogueReplicachePull(body, ctx.user);
    ...
    console.error("[replicache/catalogue/pull] failed", error);
