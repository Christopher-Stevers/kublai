...
const sourcePath = path.join(process.cwd(), "data/catalogue/xirtec-catalogue.json");
const TIM_EMAIL = process.env.XIRTEC_IMPORT_USER_EMAIL || "tim.j.saunders@gmail.com";
const MATERIALS = ["PVC Sch 40", "PVC Sch 80"];
...
async function main() {
  ...
  if (!rows.length) throw new Error("No Xirtec rows to import");
  ...
  try {
    ...
    const existingXirtec = await sql`
      select pd.id, pd."displayName", m.name as "materialName"
      from kublai_part_definition pd
      left join kublai_material m on m.id = pd."materialId"
      where pd."organizationId" = ${organizationId} and m.name = any(${MATERIALS})`;

    const backupDir = path.join(process.cwd(), "state-backups");
    fs.mkdirSync(backupDir, { recursive: true });
    const backupPath = path.join(backupDir, `xirtec-catalogue-backup-${Date.now()}.json`);
    fs.writeFileSync(
      backupPath,
      JSON.stringify({ createdAt: new Date().toISOString(), sourcePath, existingXirtec }, null, 2),
    );
    ...
    const pipeCategory = await ensureNamed(sql, "kublai_category", organizationId, "Pipe", {
      sortOrder: 0,
    });
    const fittingCategory = await ensureNamed(sql, "kublai_category", organizationId, "Fitting", {
      sortOrder: 1,
    });
    const units =
    ...
    for (const materialName of MATERIALS) {
      materialCache.set(
        materialName,
        await ensureNamed(sql, "kublai_material", organizationId, materialName, { createdAt: new Date() }),
      );
    }

    const byName = new Map(existingXirtec.map((part) => [normalizeKey(part.displayName), part.id]));
    let inserted = 0;
    let updated = 0;
    for (const [index, row] of rows.entries()) {
      ...
      const material = materialCache.get(row.material);
      if (!material) throw new Error(`Unexpected material ${row.material}`);
      const categoryId = row.category === "Pipe" ? pipeCategory.id : fittingCategory.id;
      ...
      if (existingId) {
        ...
        await sql`
          ...
          values (${partId}, ${organizationId}, ${catalog.id}, ${categoryId}, ${row.displayName}, ${normalize(row.description)}, ${normalize(row.imageUrl) || null}, ${sizeLabel}, ${material.id}, ${size.id}, true, ${new Date()})`;
      ...
      if ((index + 1) % 50 === 0 || index + 1 === rows.length) {
        console.log(`Xirtec bulk import progress: ${index + 1}/${rows.length}`);
      }
    }
