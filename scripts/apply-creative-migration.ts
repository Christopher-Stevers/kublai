...
async function applyCreativeMigration() {
  try {
    ...
    const result = await conn`
      ...
      WHERE table_name = 'genghis_creative'
      ORDER BY column_name;
    `;
    
    const existingColumns = result.map((r: { column_name: string }) => r.column_name);
    console.log("Existing columns:", existingColumns);
    
    const requiredColumns = ["userId", "fileType", "filePath", "fileSize", "mimeType", "approvedBy", "approvedAt"];
    ...
    for (const col of missingColumns) {
      ...
      if (sqlType) {
        await conn.unsafe(`ALTER TABLE "genghis_creative" ADD COLUMN IF NOT EXISTS "${col}" ${sqlType};`);
        console.log(`  ✓ Added column: ${col}`);
      }
    }
    
    // Step 2: Delete any existing rows without userId (they're invalid for new schema)
    const deleteResult = await conn`DELETE FROM "genghis_creative" WHERE "userId" IS NULL RETURNING id;`;
    if (deleteResult.length > 0) {
      console.log(`  ⚠ Deleted ${deleteResult.length} rows without userId`);
    }
    
    // Step 3: Make required columns NOT NULL
    const notNullColumns = ["userId", "fileType", "filePath", "fileSize", "mimeType"];
    for (const col of notNullColumns) {
      if (missingColumns.includes(col)) {
        await conn.unsafe(`ALTER TABLE "genghis_creative" ALTER COLUMN "${col}" SET NOT NULL;`);
