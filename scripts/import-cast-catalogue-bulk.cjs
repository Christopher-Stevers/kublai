...
const sourcePath = path.join(process.cwd(), "data/catalogue/cast-catalogue.json");
const TIM_EMAIL = process.env.CAST_IMPORT_USER_EMAIL || "tim.j.saunders@gmail.com";
...
async function main() {
  ...
  try {
    ...
    const existingCast = await sql`
      select pd.id, pd."displayName"
      from kublai_part_definition pd
      left join kublai_material m on m.id = pd."materialId"
      where pd."organizationId" = ${organizationId} and m.name = 'Cast'`;

    const backupDir = path.join(process.cwd(), "state-backups");
    fs.mkdirSync(backupDir, { recursive: true });
    const backupPath = path.join(backupDir, `cast-catalogue-backup-${Date.now()}.json`);
    fs.writeFileSync(
      backupPath,
      JSON.stringify({ createdAt: new Date().toISOString(), sourcePath, existingCast }, null, 2),
    );
    ...
    const material = await ensureNamed(sql, "kublai_material", organizationId, "Cast", {
      createdAt: new Date(),
    });
    const [inchUnit] =
    ...
    const byName = new Map(existingCast.map((part) => [normalizeKey(part.displayName), part.id]));
    let inserted = 0;
    let updated = 0;
    ...
    for (const [index, row] of rows.entries()) {
      ...
      if ((index + 1) % 50 === 0 || index + 1 === rows.length) {
        console.log(`Cast bulk import progress: ${index + 1}/${rows.length}`);
      }
    }
