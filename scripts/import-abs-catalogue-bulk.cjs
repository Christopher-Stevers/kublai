...
const sourcePath = path.join(process.cwd(), "data/catalogue/abs-catalogue.json");
...
async function main() {
  ...
  try {
    ...
    const material = await ensureNamed(sql, "kublai_material", organizationId, "ABS", {
      createdAt: new Date(),
    });
    const [unit] =
      await sql`select id from kublai_unit where lower(code) in ('in', 'inch') order by code limit 1`;
    if (!unit) throw new Error("No inch unit found");

    const byName = new Map(existingAbs.map((part) => [normalizeKey(part.displayName), part.id]));
    let inserted = 0;
    let updated = 0;
    for (const [index, row] of rows.entries()) {
      ...
      if (existingId) {
        await sql`
          ...
          set "catalogId" = ${catalog.id}, "categoryId" = ${category.id}, "displayName" = ${row.displayName},
              ...
              "sizeLabel" = ${formatSizeLabel(row.sizeNominal)}, "materialId" = ${material.id},
        ...
        await sql`
          ...
          values (${partId}, ${organizationId}, ${catalog.id}, ${category.id}, ${row.displayName}, ${normalize(row.description)}, ${normalize(row.imageUrl) || null}, ${formatSizeLabel(row.sizeNominal)}, ${material.id}, ${size.id}, true, ${new Date()})`;
      ...
      if ((index + 1) % 25 === 0 || index + 1 === rows.length) {
        console.log(`ABS bulk import progress: ${index + 1}/${rows.length}`);
      }
    }
